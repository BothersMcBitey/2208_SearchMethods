...
public class Renderer {
	...
	public Renderer(int GIF_SIZE, Problem p,  boolean showCosts) {
	...
	public BufferedImage generateFrame(Node move, boolean showMove){
		BlocksworldProblem prob = (BlocksworldProblem) p;
		int w = prob.getWidth();
		...
		g.fillRect(1 * tileW + 1, (GIF_SIZE - tileH) - (2 * tileH) + 1, tileW - 2, tileH - 2);
		...
		//node positions
		BlocksworldNode node = null;
		if(move.getParent() != null && showMove){
			node = (BlocksworldNode) move.getParent();
		} else {
			 node = (BlocksworldNode) move;
		}
		...
		for(Node child : prob.getChildren(node)){
			BlocksworldNode c = (BlocksworldNode) child;
		...
		g.fillRect(node.getB().getX() * tileW + 5, (GIF_SIZE - tileH) - (node.getB().getY() * tileH) + 5, (tileW - 10), (tileH - 10));
		g.fillRect(node.getC().getX() * tileW + 5, (GIF_SIZE - tileH) - (node.getC().getY() * tileH) + 5, (tileW - 10), (tileH - 10));
		...
		g.drawString("b", node.getB().getX() * tileW + 100, GIF_SIZE - (node.getB().getY() * tileH + 100));
		g.drawString("c", node.getC().getX() * tileW + 100, GIF_SIZE - (node.getC().getY() * tileH + 100));
